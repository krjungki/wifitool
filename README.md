...
- 선택적으로 Security 로그를 수집해 계정·로그온 실패 분석 보강
- 전체 또는 지정 SSID의 Wi-Fi 프로필을 수집 ZIP에 자동 포함
...
`수집 및 범위` 탭에서 `Security 로그 포함`을 선택할 수 있습니다. Security 로그에는 계정, SID, 로그온 상태가 포함될 수 있으므로 기본값은 해제입니다. `Wi-Fi 프로필 포함`은 기본값이 선택되어 있으며 SSID를 비워 두면 조회 가능한 모든 프로필을 수집합니다. 프로필 XML은 `keyMaterial`과 자격 증명 요소를 제거한 뒤 ZIP에 저장합니다.
